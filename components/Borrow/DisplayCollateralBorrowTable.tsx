			...
			<div className={`md:-ml-12 flex items-center ${className}`}>
				...
				<div className="flex flex-col">
					<span className={`text-left ${bold && 'font-bold'}`}>
						...
						<FontAwesomeIcon className="w-3 ml-2 cursor-pointer" icon={faArrowUpRightFromSquare} />
					</span>
					<span className="text-text-subheader text-left max-lg:w-[4rem] lg:w-[7rem] max-sm:w-[9rem] text-sm truncate">{name}</span>
				</div>
			</div>
		</Link>
	)
