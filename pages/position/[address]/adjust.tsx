...
export default function PositionAdjust() {
	...
	const collateralNote = useMemo(() => {
		...
		return collateralAmount < positionStats.collateralBal
			? t('pages:position:adjust:variables:collateralSent', { amount: amountToSend })
			: collateralAmount > positionStats.collateralBal
				? t('pages:position:adjust:variables:collateralTaken', { amount: amountToSend })
				: ''
	...
	const getCollateralError = useCallback(() => {
		if (collateralAmount - positionStats.collateralBal > positionStats.collateralUserBal) {
			return t('pages:position:adjust:variables:insufficientCollateral', { collateralSymbol: positionStats.collateralSymbol })
		} else if (liqPrice * collateralAmount < amount * 10n ** 18n) {
			return t('pages:position:adjust:variables:noEnoughCollateral')
		}
 ...
 */
	const getAmountError = useCallback(() => {
		if (amount > positionStats.limit) {
			return t('pages:position:adjust:variables:errors:limitExceeded', { limit: formatUnits(positionStats.limit, 18) })
		} else if (-paidOutAmount() > positionStats.ofdBalance) {
			return t('pages:position:adjust:variables:errors:insufficientBalance')
		} else if (liqPrice * collateralAmount < amount * 10n ** 18n) {
			return t('pages:position:adjust:variables:errors:maxMint', { maxMint: formatUnits((collateralAmount * liqPrice) / 10n ** 36n, 0) })
		} else if (positionStats.liqPrice * collateralAmount < amount * 10n ** 18n) {
			return t('pages:position:adjust:variables:errors:increaseAfterCooldown')
		} else {
			return ''
		}
	...
	return (
		<>
			<Head>
				<title>
					{envConfig.AppName} - {t('pages:position:adjust:title')}
				</title>
			</Head>
			<div>
				<AppPageHeader
					backText={t('pages:position:adjust:back')}
					backTo={`/position/${positionAddr}`}
					title={t('pages:position:adjust:title')}
				/>
				<section className="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div className="bg-gradient-to-br from-purple-900/90 to-slate-900/95 backdrop-blur-md rounded-xl p-4 flex flex-col border border-purple-500/50 gap-2">
						<div className="text-lg font-bold text-center">{t('pages:position:adjust:variables:title')}</div>
						<TokenInput
							balanceLabel={t('pages:position:adjust:variables:amountBalanceLabel')}
							error={getAmountError()}
							label={t('pages:position:adjust:variables:amount')}
							max={repayPosition}
							onChange={onChangeAmount}
							output={positionStats.closed ? '0' : ''}
							placeholder={t('pages:position:adjust:variables:amountPlaceholder')}
							symbol={CoinTicker.OFD}
							value={amount.toString()}
						/>
						<TokenInput
							balanceLabel={t('pages:position:adjust:variables:collateralBalanceLabel')}
							digit={positionStats.collateralDecimal}
							error={getCollateralError()}
							label={t('pages:position:adjust:variables:collateral')}
							max={positionStats.collateralUserBal + positionStats.collateralBal}
							note={collateralNote}
							onChange={onChangeCollAmount}
							placeholder={t('pages:position:adjust:variables:collateralPlaceholder')}
							symbol={positionStats.collateralSymbol}
							value={collateralAmount.toString()}
						/>
						<TokenInput
							balanceLabel={t('pages:position:adjust:variables:liqPriceBalanceLabel')}
							digit={36 - positionStats.collateralDecimal}
							label={t('pages:position:adjust:variables:liqPrice')}
							max={positionStats.liqPrice}
							onChange={onChangeLiqAmount}
							placeholder={t('pages:position:adjust:variables:liqPrice')}
							symbol={CoinTicker.OFD}
							...
							<GuardToAllowedChainBtn>
								{collateralAmount - positionStats.collateralBal > positionStats.collateralPosAllowance ? (
									<Button isLoading={isApproving} onClick={() => handleApprove()}>
										{t('pages:position:adjust:variables:buttons:approve')}
									</Button>
								) : (
									<Button
										disabled={
										...
										error={positionStats.owner != address ? t('pages:position:adjust:variables:buttons:adjustError') : ''}
										isLoading={isAdjusting}
										onClick={() => handleAdjust()}
										variant="primary"
									>
										{t('pages:position:adjust:variables:buttons:adjust')}
									</Button>
								)}
							</GuardToAllowedChainBtn>
						</div>
					...
					<div className="bg-gradient-to-br from-purple-900/90 to-slate-900/95 backdrop-blur-md rounded-xl p-4 flex flex-col border border-purple-500/50 gap-2">
						<div className="text-lg font-bold text-center">{t('pages:position:adjust:outcome:title')}</div>
						<div className="bg-slate-900 rounded-xl p-4 flex flex-col gap-2">
							<div className="flex">
								<div className="flex-1">{t('pages:position:adjust:outcome:minted')}</div>
								<DisplayAmount address={ADDRESS[chainId].oracleFreeDollar} amount={positionStats.minted} currency={CoinTicker.OFD} />
							</div>
							<div className="flex">
								<div className="flex-1">
									{amount >= positionStats.minted ? t('pages:position:adjust:outcome:receive') : t('pages:position:adjust:outcome:return')}
								</div>
							...
							<div className="flex">
								<div className="flex-1">
									{amount >= positionStats.minted
										? t('pages:position:adjust:outcome:reserve')
										: t('pages:position:adjust:outcome:returnFromReserve')}
								</div>
								<DisplayAmount address={ADDRESS[chainId].oracleFreeDollar} amount={returnFromReserve()} currency={CoinTicker.OFD} />
							</div>
							<div className="flex">
								<div className="flex-1">{t('pages:position:adjust:outcome:mintingFee')}</div>
								<DisplayAmount
							...
							<div className="flex font-bold">
								<div className="flex-1">{t('pages:position:adjust:outcome:futureMintedAmount')}</div>
